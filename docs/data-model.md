...
|---|---|---|---|
| `user_table` | `UserRow` | username (unique), password, email, user_id (uuid), beets_port, subsonic_port, max_library_size, bytes_used (library **and trash** bytes for the quota; NULL = not yet measured, #183, #200) | One row per user. Ports allocated via `utils/get_available_port`. |
...
| `trash_batch_table` | `TrashBatchRow` | batch_id, user_id, kind (`track`/`nodes`/`playlist_entries`), label, bytes, created_at, expires_at | One undoable delete (#200). **No state column**: its state is computed from its items (`services/trash.batch_state`). `bytes` is what it holds on disk, so the quota's trash figure is a DB read. Added in migration 022. |
| `trash_item_table` | `TrashItemRow` | batch_id, user_id, kind, state, subbox_id, relative_path, size, sha256, media_file_id, snapshot (JSON), error | One thing a batch holds, with what its restore needs: for a track, its path (the same relative path in the library and the batch's trash dir), the Navidrome id it had, and the three pymix rows the delete removed. `state`: `pending` → `restorable` → `restoring` → `restored`, or `restorable` → `expired` → `purged`; `failed`/`lost` from any. |
| `trash_reaper_run_table` | `TrashReaperRunRow` | started_at, finished_at, n_batches_purged, n_missing_swept, n_failures, errors | One row per reaper pass, so a failed pass is on record and not only in a log. |
...
| `wishlist_table` | `WishlistRow` | wishlist_id, user_id, artist/title/album/raw_note, `status` (pg enum `wishlist_status`), `metadata_source`, `resolve_state`, youtube/bandcamp/soundcloud urls, linked_subbox_id, match_confidence | Tracks the user wants but doesn't have. `status` walks `inbox → wishlist → downloaded → available → ignored`. `metadata_source='user'` locks artist/title against automatic re-matching; `resolve_state` is the async resolve loop's own work-state, and `(metadata_source, resolve_state)` is indexed for its selection query. `linked_subbox_id` + `match_confidence` are stamped by reconcile when an item flips to `available`. Lifecycle: `design-wishlist-library-automatch.md`. |
